...
async function runDiagnostics() {
  ...
  for (const tokenId of toRange(CONFIG.START_TOKEN_ID, CONFIG.END_TOKEN_ID)) {
    ...
    const blockTimestampMs = await resolveBlockTimestamp(publicClient, mintedEvent);
    const mintedAtFromContract = mintedAtRaw ? Number(mintedAtRaw) * 1000 : null;
    ...
    if (!pinataMetadata.data) {
      issues.push(
        `Metadata unavailable via Pinata gateway: ${pinataMetadata.error ?? 'unknown error'}`,
      );
    }
    ...
    if (!pinataImage.ok) {
      issues.push(`Image unavailable via Pinata gateway: ${pinataImage.error ?? 'unknown error'}`);
    }
    ...
    if (!redisSummary.exists) {
      ...
      if (metadataHash && redisSummary.metadataHash && metadataHash !== redisSummary.metadataHash) {
        issues.push(
          `Redis metadata hash (${redisSummary.metadataHash}) mismatch on-chain (${metadataHash})`,
        );
      }
      if (imageHash && redisSummary.imageHash && imageHash !== redisSummary.imageHash) {
        issues.push(
    ...
    tokens.push({
      ...
      pinata: {
        metadataUrl,
        metadataStatus: pinataMetadata.data ? 'available' : pinataMetadata.error,
        imageUrl,
...
function validateConfig(): void {
  ...
  if (
    ...
    CONFIG.CONTRACT_ADDRESS === ('0x0000000000000000000000000000000000000000' as Address)
  ) {
...
async function findTicketStampedEvent(publicClient: DiagnosticsClient, tokenId: number) {
  try {
...
async function lookupNeynarUserByAddress(address: Address): Promise<NeynarLookupResult> {
  ...
  console.log(`[diagnostic] Looking up Neynar user for address: ${normalizedAddress}`);

  const { data, error } = await fetchJson<NeynarBulkByAddressResponse>(endpoint.toString(), {
  ...
  if (data) {
    const users = data[normalizedAddress] ?? data[address] ?? data[normalizedAddress.toLowerCase()];
    if (users && users.length > 0) {
      const user = users[0];
      console.log(`[diagnostic] ✅ Found user: ${user.username} (FID: ${user.fid})`);
      return { address, user };
    }
  }

  // No user found
  console.warn(`[diagnostic] ❌ No Farcaster user found for address ${address}`);
  return {
    address,
...
function writeRepairJson(tokens: TokenDiagnostics[]): void {
  const repairTokens: RepairJsonToken[] = tokens.map((token) => ({
    ...
    onChain: {
      ...
      blockNumber: token.onChain.blockNumber ? Number(token.onChain.blockNumber) : null,
