...
async function setTicketDataOnChain(
	...
	try {
		...
		console.log(
			`[repair] Setting ticket data on-chain for token ${tokenId}...`,
		);
		const txHash = await contractService.setTicketData(
			tokenId,
			tokenURI,
			imageIPFS,
		);
...
function generateRedisCommand(tokenData: TokenData): string {
	...
	return [
		`cat <<'EOF' | redis-cli -x SET ${redisKey} NX`,
		jsonValue,
		"EOF",
	].join("\n");
}
...
async function repairToken(
	...
	try {
		...
		const nftData = await generateAndUploadNFT(
			token.tokenId,
			passengerUsername,
		);
		...
		console.error(
			`[repair] ❌ Failed to repair token ${token.tokenId}:`,
			error,
		);
		return result;
	}
...
async function runRepair() {
	...
	// Log configuration
	console.log(
		`Network: ${CONFIG.USE_MAINNET ? "Base Mainnet" : "Base Sepolia"}`,
	);
	...
	console.log(
		`Tokens to repair: ${tokensToRepair.map((token) => token.tokenId).join(", ")}\n`,
	);
