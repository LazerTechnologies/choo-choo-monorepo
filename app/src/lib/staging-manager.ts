...
export async function createStaging(
  tokenId: number,
  staging: Omit<StagingMovement, 'tokenId' | 'status' | 'createdAt' | 'retryCount' | 'version'>,
): Promise<void> {
  ...
  const created = await client.set(key, JSON.stringify(payload), 'NX', 'EX', STAGING_TTL_SECONDS);
...
export async function getStaging(tokenId: number): Promise<StagingMovement | null> {
  const key = getStagingKey(tokenId);
...
export async function abandonStaging(tokenId: number, reason: string): Promise<void> {
...
function assertPromotable(staging: StagingMovement): asserts staging is StagingMovement & {
  imageHash: string;
  metadataHash: string;
  tokenURI: string;
  txHash: string;
  ...
  for (const field of requiredFields) {
    if (!staging[field]) {
      throw new Error(`Cannot promote staging entry missing ${field as string}`);
    }
  }
}
...
function toTokenSourceType(orchestrator: OrchestratorOperation): TokenData['sourceType'] {
  switch (orchestrator) {
    case 'manual-send':
      return 'manual';
    case 'admin-send':
...
export async function promoteStaging(tokenId: number): Promise<void> {
  ...
  try {
    ...
    orchestratorLog.info(toOrchestratorLogCode(staging.orchestrator, 'promotion_store_success'), {
      tokenId: mintedTokenId,
...
export async function listStagingEntries(): Promise<StagingMovement[]> {
  ...
  do {
    try {
      const result = await redis.scan(cursor, 'MATCH', `${STAGING_KEY_PREFIX}:*`, 'COUNT', 100);
