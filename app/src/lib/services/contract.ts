...
export class ContractService {
	...
	static fromEnv(): ContractService {
		...
		if (!address || !rpcUrl) {
			throw new Error(
				"Missing required environment variables: CHOOCHOO_TRAIN_ADDRESS, RPC_URL",
			);
		}
	...
	async getTrainStatus(): Promise<TrainStatus> {
		...
		const [holder, totalStops, lastMoveTime, canBeYoinked, nextTicketId] =
			status as [Address, bigint, bigint, boolean, bigint];
	...
	async setMainTokenURI(tokenURI: string): Promise<string> {
		...
		try {
			...
			console.log(
				`[ContractService] setMainTokenURI transaction hash: ${hash}`,
			);
			return hash;
		} catch (error) {
	...
	async hasDepositedEnough(fid: number): Promise<boolean> {
		try {
			...
			console.error(
				`[ContractService] Failed to check deposit status for FID ${fid}:`,
				error,
			);
			return false;
		}
	...
	async getMintedTokenIdFromTx(txHash: `0x${string}`): Promise<number> {
		try {
			...
			console.error(
				`[ContractService] Failed to get minted token ID from tx ${txHash}:`,
				error,
			);
			...
			try {
				...
				console.error(
					`[ContractService] Transaction check failed:`,
					txCheckError,
				);
			}
			throw new Error(
	...
	async getContractInfo() {
		try {
			...
			const [totalSupply, currentHolder, totalTickets, admins] =
				await Promise.all([
