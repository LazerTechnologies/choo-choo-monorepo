...
export async function executeTrainMovement(
  ...
  if (existingStaging.status === 'completed') {
    ...
    try {
      ...
      orchestratorLog.error(toOrchestratorLogCode(operation, 'promotion_failed'), {
        tokenId: initialTokenId,
        error,
      });
      ...
      return {
        ...
        txHash: '',
        tokenURI: '',
        error: error instanceof Error ? error.message : 'Failed to promote completed staging entry',
      };
    }
  ...
  try {
    ...
    try {
      ...
      orchestratorLog.info(toOrchestratorLogCode(operation, 'contract_confirmed'), {
    ...
    if (needsMetadataOnchain && metadataOperation) {
      try {
        ...
        orchestratorLog.error(toOrchestratorLogCode(operation, 'metadata_set'), {
          tokenId: mintedTokenId,
          error,
    ...
    try {
      ...
      return {
        ...
        error: error instanceof Error ? error.message : 'Failed to promote staging entry',
      };
    }
    ...
    try {
      await verifyNextIdAdvanced(contractService, mintedTokenId, `orchestrator-${operation}`);
    } catch (verifyError) {
    ...
    if (finalStaging?.needsMetadataRetry) {
      orchestratorLog.warn(toOrchestratorLogCode(operation, 'post_commit_warning'), {
        tokenId: mintedTokenId,
...
export async function orchestrateManualSend(
  ...
  try {
    ...
    if (currentContractHolder.toLowerCase() === targetAddress.toLowerCase()) {
      throw new Error(`Cannot send train to current holder ${user.username} (${targetAddress})`);
    }

    orchestratorLog.info(toOrchestratorLogCode('manual-send', 'staging_updated'), {
      tokenId: nextTokenId,
    ...
    const movement = await executeTrainMovement({
      ...
      contractOperation: async () => {
        ...
        if (!mintJson.success) {
          throw new Error(`Token minting failed: ${mintJson.error || 'Unknown error'}`);
        }
    ...
    try {
      ...
      if (!welcomeResponse.ok) {
        ...
        orchestratorLog.warn(toOrchestratorLogCode('manual-send', 'post_commit_warning'), {
...
export async function orchestrateRandomSend(castHash: string) {
  ...
  try {
    ...
    // Check if trying to send to current holder
    const currentContractHolder = await contractService.getCurrentTrainHolder();
    if (currentContractHolder.toLowerCase() === winnerData.winner.address.toLowerCase()) {
      throw new Error(
    ...
    try {
      ...
      if (!welcomeResponse.ok) {
        const errorData = await welcomeResponse.json();
        orchestratorLog.warn(toOrchestratorLogCode('random-send', 'post_commit_warning'), {
          tokenId: movement.tokenId,
          stage: 'welcome_cast',
          status: welcomeResponse.status,
          error: errorData.error,
...
export async function orchestrateYoink(userFid: number, targetAddress: string) {
  ...
  try {
    ...
    const movement = await executeTrainMovement({
      ...
      contractOperation: async () => {
        const txHash = await contractService.executeYoink(targetAddress as `0x${string}`);
        const mintedTokenId = (await contractService.getMintedTokenIdFromTx(txHash)) ?? nextTokenId;
    ...
    try {
      ...
      if (!welcomeResponse.ok) {
        const errorData = await welcomeResponse.json();
        orchestratorLog.warn(toOrchestratorLogCode('yoink', 'post_commit_warning'), {
          tokenId: movement.tokenId,
          stage: 'welcome_cast',
          status: welcomeResponse.status,
          error: errorData.error,
