...
export async function POST(request: Request) {
	try {
		...
		if (process.env.NEYNAR_WEBHOOK_SECRET) {
			...
			if (
				!validateWebhook(rawBody, signature, process.env.NEYNAR_WEBHOOK_SECRET)
			) {
		...
		if (body.type === "cast.created") {
			...
			try {
				const embeds: Array<Record<string, unknown>> = Array.isArray(
					cast.embeds,
				)
				...
				containsChoochooEmbed = embeds.some((e: Record<string, unknown>) => {
					...
					const url =
						typeof candidate === "string" ? candidate.toLowerCase() : undefined;
					return (
						!!url && (url.includes("choochoo.pro") || url.includes("choochoo"))
					);
				});
			} catch {}
			console.log(
			...
			if (containsChoochoo || containsChoochooEmbed) {
				...
				try {
					...
					console.log(
						...
						JSON.stringify(currentHolder, null, 2),
					);
				} catch (parseError) {
					console.error(
				...
				console.log(
					`🔍 [webhook]   FIDs match: ${currentHolderFid === castAuthorFid}`,
				);
				...
				if (currentHolderFid === castAuthorFid) {
					...
					try {
						...
						console.log(
							`✅ [webhook] Updated workflow state to CASTED with hash ${cast.hash}`,
						);
					...
					console.log(
						`ℹ️ [webhook] Cast author: ${cast.author.username} (FID: ${castAuthorFid})`,
					);
