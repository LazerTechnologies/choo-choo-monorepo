	...
	try {
		...
		if (strategy === "choochoo") {
			...
			for (const query of searchQueries) {
				try {
					...
					if (searchResponse.ok) {
						...
						if (casts.length > 0) {
							console.log(
								`[cast-data/fallback] Found ${casts.length} casts for query "${query}"`,
							);
							return NextResponse.json({
					...
					console.log(
						`[cast-data/fallback] Query "${query}" failed:`,
						queryError,
					);
				}
			}
			...
			try {
				...
				if (userCastsResponse.ok) {
					...
					const castWithAppUrl = casts.find((cast: any) =>
						cast.embeds?.some(
							...
							(embed: any) =>
								embed.url &&
								(embed.url.includes(APP_URL) ||
									embed.url.includes("choochoo.pro")),
						),
					);
				...
				console.log(
					"[cast-data/fallback] APP_URL embed search failed:",
					embedError,
				);
			}

			return NextResponse.json(
				{ error: "No ChooChoo-related casts found" },
				{ status: 404 },
			);
		}
		...
		if (strategy === "recent") {
			...
			try {
				...
				console.error(
					"[cast-data/fallback] Recent cast search failed:",
					recentError,
				);
			}

			return NextResponse.json(
				{ error: "No recent casts found" },
				{ status: 404 },
			);
		}
