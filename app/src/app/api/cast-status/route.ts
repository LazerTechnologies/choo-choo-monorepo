...
export async function GET(request: Request) {
	...
	try {
		...
		console.log(
			`🔍 [cast-status] Retrieved workflow state from Redis:`,
			workflowStateJson,
		);
		...
		if (workflowStateJson) {
			...
			console.log(
				`🔍 [cast-status] Current cast hash: ${workflowData.currentCastHash}`,
			);

			if (hasCurrentUserCasted && workflowData.currentCastHash) {
				console.log(
					`✅ [cast-status] Returning positive cast status for FID ${fid}`,
				);
			...
			console.log(
				`ℹ️ [cast-status] No workflow state found in Redis - checking API fallback`,
			);
		}
		...
		if (process.env.NEYNAR_API_KEY) {
			...
			try {
				...
				if (response.ok) {
					...
					for (const cast of casts) {
						...
						const castDate =
							ts && (typeof ts === "string" || typeof ts === "number")
								? new Date(ts)
								: new Date(0);
						...
						const preview =
							typeof rawText === "string" ? rawText.substring(0, 50) : "";
						console.log(
						...
						if (castDate > fifteenMinutesAgo) {
							...
							const author = (cast as { author?: unknown }).author as
								| { fid?: unknown }
								| undefined;
							const fidStr =
								author && typeof author.fid === "number"
									? String(author.fid)
									: "unknown";
							console.log(
				...
				console.error(
					"🚨 [cast-status] Error checking recent casts:",
					apiError,
				);
			...
			console.warn(
				"⚠️ [cast-status] NEYNAR_API_KEY not configured - fallback detection disabled",
			);
		}

		console.log(
			`🚨 [cast-status] No cast found for FID ${fid} - returning negative result`,
		);
		return NextResponse.json({
		...
		return NextResponse.json(
			{ error: "Failed to check cast status" },
			{ status: 500 },
		);
	}
}
