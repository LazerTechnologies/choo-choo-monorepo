		...
		<Card className={className}>
			<Card.Content>
				<div className="space-y-2">
					...
					{depositHook.error && (
						<div className="text-xs text-red-500">{depositHook.error}</div>
					)}
					<Button
						onClick={onClick}
						disabled={disabled}
					...
					{!satisfied && config && (
						<Typography variant="small" className="text-gray-600">
							Required: {Number(required) / 10 ** config.decimals} USDC •
							Deposited: {Number(deposited) / 10 ** config.decimals} USDC
						</Typography>
					)}
				</div>
			</Card.Content>
